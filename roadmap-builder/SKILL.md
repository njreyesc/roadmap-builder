...
1. **Получение `roadmap.json`** — зависит от режима:
   ...
   - **Режим B (список фич с оценками):** собери `features.json` по схеме `references/features-schema.md` (фичи, оценки фаз в неделях/спринтах, опционально исполнители, зависимости `after`, вехи) и прогони планировщик:
     ```bash
...
```
Понимает подписанную форму (`Фича: аналитика 2 нед, разработка 3 спринта, тестирование 2 нед`, метки — слово или буква А/Р/Т) и позиционную (`Фича: 2 нед, 3 спринта, 2 нед` или через `|`/`;`, пустая ячейка = фазы нет). Поддержка `xN` (ше) и `/ Имя` (исполнитель) в оценке. Порядок строк = приоритет. Схема результата — та же `features.json`.
...
Типичный вход: согласованный и приоритизированный список фич; по каждой — оценки аналитики, разработки, тестирования (недели или спринты), опционально исполнители и финальная веха («Выход на КО»). Источник — слова пользователя, таблица или выгрузка из Jira (см. выше).

- Оценки бери из слов пользователя как есть; единицу (недели/спринты), длину спринта (`sprint_weeks`, по умолчанию 2) и капасити команды уточни, если неочевидно — одним вопросом вместе с датой старта.
- Фазы фичи идут строго последовательно и **встык, без разрывов**: если пул какой-то фазы занят, сдвигается вся фича целиком (аналитика не делается «в стол» задолго до разработки). Фичи между собой планируются по капасити: `capacity` (число — общий пул ше, объект `{analytics, dev, testing}` — пулы по стадиям) определяет пропускную способность, фичи занимают ресурсы в порядке списка, не влезающие — сдвигаются (планировщик печатает сдвиги). Без `capacity` фичи параллельны. Разрывы между фазами можно разрешить `"allow_gaps": true`. Жёсткую последовательность задавай `after` (или явным `start`).
- Раскладка по людям опциональна: `who` в оценке фазы попадает в подпись полосы, `owner` фичи — в левую колонку, `people` — сколько ше фаза занимает в пуле. Без них roadmap строится так же.
